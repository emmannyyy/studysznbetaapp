...
with st.form("question_choice", clear_on_submit=False):
	...
	with left_column:	
		chosen_question = st.number_input(label="Choose a question from 1 to 420",min_value=1,max_value=420)
	...
	if st.session_state.similarQs > 0:
		...
		with question_expander:
			st.markdown((parseQuestion(df,chosen_question)))
			st.session_state.similarQs +=1
...
if st.session_state.similarQs>0:
	if st.button("View Similar Questions"):
        ...
        # Assuming you have defined similar_question_indices using find_most_similar_questions
...
#		with tab1:
#			question_expander_1 = st.expander("Click to view question")
#			with question_expander_1:
#				st.markdown(parseQuestion(df,similar_question_indices[0]))
				
#			submitAnswer_1 = st.expander("Click to view Answers")
#			with submitAnswer_1:
						
#				html_with_css += parseAnswer(df, similar_question_indices[0])
