...
with st.form("question_choice", clear_on_submit=False):
    ...
    with right_column:
        if st.session_state.current_question < 0:
            ...
            st.info(f"Current Selected Question: {chosen_question + 1}  \nTopics tested: {get_topics(chosen_question)}", icon="📔")
    ...
    if st.button("View Similar Questions"):
        ...
        for i,tab in enumerate(tabs):
            with tab:
                question_expander = st.expander("Click to view question")
                with question_expander:
                    question_output = html_with_css + parseQuestion(df,similar_question_indices[i])
                    st.markdown(question_output,unsafe_allow_html=True)

                submitAnswer = st.expander("Click to view Answers")
                with submitAnswer:
                    answer_output = html_with_css + parseAnswer(df, similar_question_indices[i])
